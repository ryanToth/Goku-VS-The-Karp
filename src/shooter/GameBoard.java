...
public class GameBoard extends JPanel implements ActionListener {
        ...
        addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                if (code  == KeyEvent.VK_P) {
                    ...
                    while (JOptionPane.showOptionDialog(null, "", "Pause",
                ...
                for (int j = 0; j < enemies.get(i).numberOfEnemies; j++) {
                    for (int k = 0; k < gunShip.bullets.size(); k++) {
                        if (enemies.get(i).enemies[j] != null && gunShip.bullets.get(k).getBounds().intersects(enemies.get(i).enemies[j].getBounds())
                            ...
                            if (!gunShip.bullets.get(k).didExplode) {
                                gunShip.bullets.get(k).didExplode = true;
                                shotsHit++;
                            }
    ...
    public void gameOver() throws FileNotFoundException, UnsupportedEncodingException {
        ...
        while (choice != 2) {
            ...
            if (gunShip.shotsFired != 0) percent = shotsHit*100/gunShip.shotsFired;
            
            choice = JOptionPane.showOptionDialog(null, " Your score was " + score + "\nYour hit percentage was " + percent +
                    "% with " + gunShip.shotsFired + " shots fired", "Game Over",
